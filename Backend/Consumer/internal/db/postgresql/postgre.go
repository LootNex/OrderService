...
func InitPosgres(config *config.Config, logger *zap.Logger) (*sql.DB, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("cannot open posgres err:%v", err)
	}

	if err = db.Ping(); err != nil {
		return nil, fmt.Errorf("ping err:%v", err)
	}
	logger.Info("Postgres is running")
